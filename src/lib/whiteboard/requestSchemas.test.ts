...
describe('requestSchemas hardening (SEC-005)', () => {
  describe('roomSceneSchema', () => {
    ...
    describe('document element contract', () => {
      ...
      it('accepts the document type only with bounded reference fields', () => {
        expect(roomSceneSchema.safeParse({
          elements: [documentElement()],
        }).success).toBe(true);
        expect(roomSceneSchema.safeParse({
          elements: [documentElement({ label: 'Week 1 slides' })],
        }).success).toBe(true);
      });

      it('accepts values at the bounded limits', () => {
        expect(roomSceneSchema.safeParse({
          elements: [documentElement({
            boardId: 'b'.repeat(128),
      ...
      it('rejects a document element carrying extra keys', () => {
        const outcome = parseBody(roomSceneSchema, {
          elements: [documentElement({
        ...
        expect(outcome.ok).toBe(false);
        if (!outcome.ok) {
          expect(outcome.error).toContain('document element carries only bounded reference fields');
        }
        expect(roomSceneSchema.safeParse({
          elements: [documentElement({ dataUrl: 'data:application/pdf;base64,JVBERi0xLjQ=' })],
        }).success).toBe(false);
        expect(roomSceneSchema.safeParse({
          elements: [documentElement({ link: '/rooms/r/documents/d/original' })],
        }).success).toBe(false);
      });

      it('rejects oversized or non-string reference fields', () => {
        for (const key of ['boardId', 'instanceId', 'documentId']) {
          const outcome = parseBody(roomSceneSchema, {
            elements: [documentElement({ [key]: 'r'.repeat(129) })],
          });
          expect(outcome.ok).toBe(false);
          if (!outcome.ok) {
            expect(outcome.error).toContain(`document element ${key} must be a string of 1 to 128 characters`);
          }
          expect(roomSceneSchema.safeParse({
            elements: [documentElement({ [key]: 42 })],
          }).success).toBe(false);
          expect(roomSceneSchema.safeParse({
            elements: [documentElement({ [key]: '' })],
          }).success).toBe(false);
        }
      });

      it('rejects a negative or fractional sharedPageIndex', () => {
        const negative = parseBody(roomSceneSchema, {
          elements: [documentElement({ sharedPageIndex: -1 })],
        });
        expect(negative.ok).toBe(false);
        if (!negative.ok) {
          expect(negative.error).toContain('document element sharedPageIndex must be a non-negative integer');
        }
        expect(roomSceneSchema.safeParse({
          elements: [documentElement({ sharedPageIndex: 1.5 })],
        }).success).toBe(false);
        expect(roomSceneSchema.safeParse({
          elements: [documentElement({ sharedPageIndex: 'two' })],
        }).success).toBe(false);
        const missingPage = documentElement();
        delete (missingPage as Record<string, unknown>).sharedPageIndex;
        expect(roomSceneSchema.safeParse({
          elements: [missingPage],
        }).success).toBe(false);
      });
      ...
      it('rejects non-finite geometry', () => {
        for (const key of ['x', 'y', 'width', 'height', 'angle']) {
          const outcome = parseBody(roomSceneSchema, {
            elements: [documentElement({ [key]: Number.NaN })],
          });
          expect(outcome.ok).toBe(false);
          if (!outcome.ok) {
            expect(outcome.error).toContain(`document element ${key} must be a finite number`);
          }
          expect(roomSceneSchema.safeParse({
            elements: [documentElement({ [key]: Number.POSITIVE_INFINITY })],
          }).success).toBe(false);
        }
      });
      ...
      it('rejects a non-integer version', () => {
        const outcome = parseBody(roomSceneSchema, {
          elements: [documentElement({ version: 'three' })],
        });
        expect(outcome.ok).toBe(false);
        if (!outcome.ok) expect(outcome.error).toContain('document element version must be an integer');
        expect(roomSceneSchema.safeParse({
          elements: [documentElement({ version: Number.NaN })],
        }).success).toBe(false);
        const missingVersion = documentElement();
        delete (missingVersion as Record<string, unknown>).version;
        expect(roomSceneSchema.safeParse({
          elements: [missingVersion],
        }).success).toBe(false);
      });

      it('rejects an oversized or non-string label', () => {
        const outcome = parseBody(roomSceneSchema, {
          elements: [documentElement({ label: 'x'.repeat(201) })],
        });
        expect(outcome.ok).toBe(false);
        if (!outcome.ok) {
          expect(outcome.error).toContain('document element label must be a string of at most 200 characters');
        }
        expect(roomSceneSchema.safeParse({
          elements: [documentElement({ label: 42 })],
        }).success).toBe(false);
      });
    });
  });
  ...
  describe('grammar anchors and messages', () => {
    ...
    it('tags every scene issue with the custom code', () => {
      ...
      const cases: unknown[] = [
        ...
        { elements: [{ id: 'el-1', type: 'rectangle', link: 'http://example.com' }] },
        { elements: [{ id: 'doc-1', type: 'document', manifest: {} }] },
        { elements: [{ id: 'doc-1', type: 'document', boardId: 42 }] },
        { elements: [{ id: 'doc-1', type: 'document', sharedPageIndex: -1 }] },
        { elements: [{ id: 'doc-1', type: 'document', x: Number.NaN }] },
        { elements: [{ id: 'doc-1', type: 'document', version: 1.5 }] },
        { elements: [{ id: 'doc-1', type: 'document', label: 42 }] },
      ];
