 ...
 */
/**
 * Reads the message type (leading varint) from a binary frame.
 * Returns null if the frame is empty or too short/malformed to decode.
 */
export function getFrameMessageType(bytes: Uint8Array): number | null {
  if (bytes.length === 0) {
    return null;
  }
...
export function isRelayableFrame(bytes: Uint8Array): boolean {
  const messageType = getFrameMessageType(bytes);
