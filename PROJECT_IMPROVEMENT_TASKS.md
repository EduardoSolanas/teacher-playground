...
| PERF-S2 | Size / load | Split the 2.9 MB Excalidraw fork chunk (per-tool dynamic imports, deferred non-core pieces). TypeSafe: impact 2.7/4, effort 2.4/4, worth-now 0.61. | `.next` chunk listing | Fold into the next pinned fork release batch � no longer paired with document work (spec/PDF_IMPORT_SPEC.md needs no fork change) | Open | L |
