...
| PERF-S2 | Size / load | Split the 2.9 MB Excalidraw fork chunk (per-tool dynamic imports, deferred non-core pieces). TypeSafe: impact 2.7/4, effort 2.4/4, worth-now 0.61. | `.next` chunk listing | Fold into the next pinned fork release batch � no longer paired with document work (spec/PDF_IMPORT_SPEC.md needs no fork change). **Repo-side portion done (verified)**: `PdfImportDialog` is now a `next/dynamic` boundary in `RoomClient.tsx`; red test `keeps PdfImportDialog and pdfjs-dist out of the room entry chunk (PERF-S2)` listed `components/whiteboard/PdfImportDialog.tsx` in the static closure, then passed after the boundary; targeted mutant (static import restored) killed by that assertion; webpack client stats: zero PDF modules in any initial chunk (nested walk); room page entry 131,579 to 126,102 B; dialog + pdfRenderer moved to a 7,282 B async chunk fetched only after a file pick; e2e `pdf-import.spec.ts` 6/6 green inside the full suite (220 passed, 0 failed). **The 2.9 MB split cannot be done by webpack**: chunk `43270263` (2,968,419 B) and `d401260b` (1,819,361 B) each contain exactly one module (fork `dist/prod/chunk-ZMV7GT6H.js`, `chunk-EIO257PC.js`); `splitChunks.cacheGroups` groups modules but cannot divide a single pre-built file, so the split stays batched with the next pinned fork release (per-tool dynamic imports in fork source). Correction: the 1.82 MB chunk is fork code, not LiveKit (`livekit-client` is its own 516,612 B async chunk); pdfjs was already async (1,184,269 B worker, 433,050 B pdf.mjs on dialog mount) | Open | L |
| PERF-S3 | Connectivity (rejected) | y-webrtc P2P board sync � **rejected by design**: board updates must keep flowing through the RoomDO, where authorization, persistence, snapshots and moderation live. TypeSafe concurred: impact 0.8/4, effort 3.7/4, worth-now 0.14. | README.md "Collaboration and signaling" | None � non-goal. Do not revisit without a security-design review | Closed (rejected) | � |
