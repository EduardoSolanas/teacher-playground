...
The MVP is the **PDF vertical**: upload → convert → place → shared/local page navigation → authorized reads, shipped end to end under every section that applies to it. PPTX and DOCX extend the identical pipeline, then PDF export, then Drive and OneDrive import. The feature is not accepted as complete until all three formats meet the same authorization, rendering, annotation, export, accessibility, and recovery criteria, but the PDF vertical is shippable on its own before they exist. Legacy `.ppt` and `.doc`, editable Office content, embedded media playback, macros, forms, password-protected files, and cross-room document reuse are out of scope.

**Milestone 0 precedes every pipeline commitment:** the converter-runtime spike (§5.1) selects the isolated execution environment for conversion, and its outcome may reshape the defaults in §4, §5, and §6. Those sections are provisional until the spike lands.
...
Full security requirements are normative in [SECURITY_FEATURE_REQUIREMENTS.md](../SECURITY_FEATURE_REQUIREMENTS.md), under **Embedded paginated documents (PDF, PPTX, DOCX)**. Governance: that document is the single normative owner of security requirements; this document is the single owner of product and integration behavior. Where they appear to conflict on a security topic, the security document wins and this document must be corrected. Neither document restates the other's content; they cross-reference by section title, never line numbers.
...
Conversion runs in an isolated asynchronous job environment with explicit CPU, memory, wall-time, filesystem, and network limits. Do not assume an ordinary Cloudflare Worker can execute LibreOffice or another native renderer. No converter vendor, library, hosted service, or runtime is selected by this specification, and **the runtime itself is the first decision, not the last**: milestone 0 (§10) spikes the isolated environment — self-hosted sidecar, container platform, or in-runtime JS rendering — and the pipeline defaults in §4–§6 are provisional until it lands. Selection requires a tested spike covering visual fidelity, font substitution, equations, charts, transparency, rotated text, notes/hidden slides, page sizing, malformed inputs, deterministic output, cancellation, cold start, cost (compute and R2 storage/operations), observability, data location, retention, and failure cleanup.
...
Progress reports state and coarse bounded counts such as pages completed/total. The MVP polls through an authorized room route at the §6 cadence; push through the room channel is a later optimization and must not become a second source of truth for state. The UI distinguishes uploading, queued, converting, ready, retryable failure, terminal unsupported/corrupt failure, offline, and removed states. A failed conversion retains the original only for the documented retry/erasure window and never leaves a selectable blank object indefinitely.
...
| Conversion wall time | Per format, set by the spike; proposed defaults 120 s (PDF) and 600 s (PPTX/DOCX) per attempt | A 200-page DOCX through a cold native converter does not fit 120 s. Timeout is a retryable classification only when the job is idempotent. |
...
| Browser page cache | 64 MiB decoded globally per board and 32 MiB encoded globally | The budget applies even when several document instances are visible: prioritize selected/in-viewport pages, show placeholders for evicted pages, and never pin every full-resolution current page. Previous/next pages and thumbnails may be prefetched only inside the global budgets and are first to be evicted; there is no per-instance multiplication of the cap. Reduced-resolution display variants are a separate later-phase subsystem (§12), not part of this budget's design. |
...
Undo is actor-scoped. **The bridging mechanism is an open design decision, not a given:** the editor's history is element-snapshot based, so semantic undo of a server-side manifest operation (insert, remove, restore with grace-period interplay) does not map onto it for free. Milestone design must choose and prove one of at least these shapes: (a) a per-owner document-operation journal that the editor history replays through server-confirmed transitions, or (b) history integration for scene-visible geometry only, with document lifecycle undo expressed as an explicit grace-window restore action. Whichever shape is chosen, the invariants below hold without exception:
...
0. **Converter runtime spike and rollout contract.** Before any pipeline code: evaluate the isolated conversion runtime against the §5.1 criteria on real PDF, PPTX, and DOCX fixtures, and write the flag contract — a server-side flag gates the entire surface (upload, conversion, placement); the kill switch stops new uploads and conversions immediately while existing ready documents remain readable; the flag state never weakens the authorization checks. The spike's recorded result is a prerequisite for reviewing the §4–§6 defaults.
...
3. **Isolated conversion.** Use real malicious and representative fixture files. Workerd tests such as `documentConversion.workers.test.ts › redelivery is idempotent`, `› stale lease cannot publish`, `› publishes only a complete validated WebP revision`, and `› cancellation fences late output` verify the job protocol and publication boundary; they must not pretend to execute a native converter. The MVP milestone covers PDF end to end (§1); PPTX and DOCX repeat this milestone against their own fixtures rather than being waved through by PDF's results. A separate integration suite in the selected isolated job runtime must invoke the real converter and prove page count, geometry, hostile-input handling, and accepted visual-diff thresholds per format. No fake converter may make either acceptance layer green.
...
The **PDF vertical MVP** is accepted when: the PDF path passes representative fidelity fixtures; one document remains one scene object; originals and pages stay outside the collaborative scene; owner-only metadata is enforced on HTTP and Yjs paths; authorized readers can view pages without public storage; annotations remain page-stable through transform and navigation; local browsing never changes the shared page; conversion is isolated, bounded, retryable, observable, and idempotent; the flag and kill-switch contract from milestone 0 works; all required suites and mutations pass; the pinned fork release is reproducible; independent verification returns `APPROVE`; and the security requirements referenced in section 1 have implementation evidence for every shipped surface.

The **complete feature** adds, for PPTX and DOCX: their own fidelity fixtures and milestone-3 evidence, plus undo, erasure, clipboard, accessibility, mobile, and fullscreen behavior matching this specification across all three formats.

## 12. Deferred after MVP

Explicitly not part of the PDF vertical, each requiring its own design note before implementation: reduced-resolution display variants for the browser page cache (a second rendering pipeline, not a cache-policy tweak); push-based conversion progress through the room channel; Drive and OneDrive import (§1); the all-pages lesson-archive export format (§9).
